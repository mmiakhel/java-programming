...
Requirement: any numbers that are next to each other are not single digit numbers but
instead read as a full number
    Ex: a5bc12def100g
    ...
    public static int getSumFromString(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if(Character.isDigit(str.charAt(i))) { //checks if every character is a number(digit)
                ...
                if(i == str.length()-1 || !Character.isDigit(str.charAt(i +1))) { //the order of this matter

                    sum += Integer.parseInt(num);
                    num = ""; //resets our String for the next number
